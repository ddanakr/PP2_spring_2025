...
class Fruit:
    ...
    def randomize(self, snake_body):
        self.weight = random.randrange(100, 500, 100)
        self.spawn_time = pygame.time.get_ticks() # Store spawn time
        self.despawn_time = self.spawn_time + 7000 # Dissapear after 7 sec
...
class Main():
    ...
    def check_respawn_time(self): # Check the current time with the stored spawn time
        if pygame.time.get_ticks() > self.fruit.despawn_time:
            self.fruit.randomize(self.snake.body)
